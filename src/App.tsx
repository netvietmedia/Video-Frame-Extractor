...
// ---- An toàn cho TypeScript & runtime GitHub Pages (không crash nếu không có window.aistudio) ----
declare global {
  interface Window {
    aistudio?: {
...
const AppContent: React.FC = () => {
  ...
  // Không crash trên Pages nếu không có window.aistudio
  useEffect(() => {
    const check = async () => {
      try {
  ...
  const handleRewriteScript = useCallback(async () => {
    ...
    // Nếu chạy trong AI Studio thì mở chọn key, còn trên Pages thì bỏ qua (không crash)
    try {
      ...
      // không chặn tiếp, vẫn cho chạy nếu có env key
    }

    // Lấy API key từ Vite env
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY as string | undefined
    if (!apiKey) {
      setStatusMessage(t('status.error.apiKeyNotFound') || 'Missing VITE_GEMINI_API_KEY')
      return
    }
    ...
    try {
      ...
      const imageParts = await Promise.all(
        extractedFrames.map(async (frame) => {
          const base64Data = await blobToBase64(frame.blob) // cần là raw base64, KHÔNG kèm prefix
          return { inlineData: { mimeType: 'image/jpeg', data: base64Data } }
        })
      )

      const stream = await model.generateContentStream({
        contents: [{ role: 'user', parts: [{ text: detailedPrompt }, ...imageParts] }]
      })
  ...
  const handleSaveSceneJson = () => {
    ...
    try {
      JSON.parse(editableSceneJsonString)
  ...
  const handleVoiceSelection = (voiceName: string, isChecked: boolean) => {
    setSelectedVoices((prev) => (isChecked ? [...prev, voiceName] : prev.filter((v) => v !== voiceName)))
  }
      ...
      <div className="w-full max-w-4xl mx-auto bg-gray-800 rounded-2xl shadow-2xl p-6 md:p-8 space-y-8 relative">
        ...
        <section className="p-6 bg-gray-700/50 rounded-lg border border-gray-600 space-y-6">
              ...
              <div className="flex items-center gap-4">
                <label>
                  <input
                    ...
                    name="mode"
                    value="count"
                    checked={extractionMode === 'count'}
                    onChange={() => setExtractionMode('count')}
                    className="mr-2 accent-cyan-500"
                    disabled={isProcessing}
                  />
                  {t('config.analysis.modeCount')}
                </label>
                <label>
                  <input
                    type="radio"
                    name="mode"
                    value="interval"
                    checked={extractionMode === 'interval'}
                    onChange={() => setExtractionMode('interval')}
                    className="mr-2 accent-cyan-500"
                    disabled={isProcessing}
                  />
                  {t('config.analysis.modeInterval')}
                </label>
              </div>
              {extractionMode === 'count' ? (
                <div>
                  <label htmlFor="frameCount" className="block text-sm font-medium text-gray-300">
                    {t('config.analysis.frameCountLabel')} <span className="font-bold text-cyan-400">{frameCount}</span>
                  </label>
                  <input
                    type="range"
                    id="frameCount"
                ...
                <div>
                  <label htmlFor="interval" className="block text-sm font-medium text-gray-300">
                    {t('config.analysis.intervalLabel')}
                  </label>
                  <input
                    type="number"
                    id="interval"
                    ...
                    min="0.1"
                    step="0.1"
                    className="w-full mt-1 bg-gray-900 border border-gray-600 rounded-md p-2 text-white"
                    disabled={isProcessing}
                  />
                </div>
              )}
          ...
          <div>
            ...
            {!videoFile ? (
              <label
                htmlFor="file-upload"
                className="flex justify-center items-center px-6 pt-5 pb-6 border-2 border-gray-500 border-dashed rounded-md h-full cursor-pointer hover:bg-gray-700/20 transition-colors"
              >
              ...
              <div className="bg-gray-800 p-4 rounded-md h-full flex flex-col justify-center">
                ...
                {videoInfo && (
                  <div className="mt-4 text-xs text-gray-400 grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2">
                    ...
                    {videoInfo.frameRate && (
                      <div>
                        <strong>Frame Rate:</strong> {videoInfo.frameRate.toFixed(2)} fps
                      </div>
                    )}
                    {videoInfo.dataRate && (
                      <div>
                        <strong>Data Rate:</strong> {videoInfo.dataRate} kbps
                      </div>
                    )}
                    {videoInfo.totalBitrate && (
                      <div>
                        <strong>Total Bitrate:</strong> {videoInfo.totalBitrate} kbps
                      </div>
                    )}
                    {videoInfo.suggestedFrames && (
                      <div className="font-semibold text-cyan-400">
                        <strong>{t('results.frames.suggested')}:</strong> {videoInfo.suggestedFrames}
                      </div>
                    )}
                  </div>
                )}
                <button
                  onClick={resetState}
                  className="mt-4 flex items-center justify-center gap-2 text-sm text-red-400 hover:text-red-300 disabled:text-gray-500 self-start"
                  disabled={isProcessing}
                >
                  <TrashIcon className="w-4 h-4" /> {t('config.upload.delete')}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <button
            ...
            className="w-full py-3 px-6 text-lg font-bold text-white bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-600 rounded-md transition-colors"
          >
        ...
        {(isProcessing || statusMessage) && (
          <div className="space-y-3 pt-4">
            <p className={`text-center font-medium ${statusMessage.includes(t('common.error')) ? 'text-red-400' : 'text-cyan-300'}`}>
              {statusMessage}
            </p>
        ...
        {extractedFrames.length > 0 && (
          <div className="space-y-8 pt-4 border-t border-gray-700">
            <section>
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  disabled={isProcessing}
                  className="flex items-center gap-2 text-sm bg-gray-700 hover:bg-gray-600 px-3 py-1.5 rounded-md disabled:opacity-50 transition-colors"
                >
              ...
              <div className="max-h-96 overflow-y-auto bg-gray-900/50 p-4 rounded-lg grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
                {extractedFrames.map((frame, index) => (
                  <img
                    key={index}
                    src={frame.url}
                    alt={`${t('results.frames.alt')} ${index + 1}`}
                    className="w-full h-auto object-cover rounded aspect-video bg-gray-700"
                    loading="lazy"
                    decoding="async"
                  />
                ))}
            ...
            <section className="p-6 bg-gray-700/50 rounded-lg border border-gray-600 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="sceneCount" className="block text-sm font-medium text-gray-300 mb-1">
                      {t('config.script.sceneCount')}
                    </label>
                    <input
                      type="number"
                      id="sceneCount"
                      value={sceneCount}
                      onChange={(e) => setSceneCount(Math.max(1, parseInt(e.target.value, 10)) || 1)}
                      min="1"
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-8 pt-1">
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={hasBackgroundMusic}
                        onChange={(e) => setHasBackgroundMusic(e.target.checked)}
                        className="accent-cyan-500 w-4 h-4"
                        disabled={isProcessing}
                      />{' '}
                      {t('config.script.backgroundMusic')}
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={!hasDialogue || isProcessing}
                      className="w-full bg-gray-800 border border-gray-600 rounded-md p-2 text-white disabled:bg-gray-600 disabled:text-gray-400"
                    >
                      ...
                      {availableVoices.map((voice) => (
                        <label key={voice.name} className="flex items-center gap-2 text-sm text-gray-300">
                          <input
                            ...
                            onChange={(e) => handleVoiceSelection(voice.name, e.target.checked)}
                            className="accent-cyan-500 w-4 h-4"
                            disabled={isProcessing}
                          />
              ...
              <div>
                ...
                {!srtFile ? (
                  <label
                    htmlFor="srt-upload"
                    className="flex justify-center items-center px-6 py-4 border-2 border-gray-500 border-dashed rounded-md cursor-pointer hover:bg-gray-700/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <div className="space-y-1 text-center">
                  ...
                  <div className="bg-gray-800 p-3 rounded-md flex items-center justify-between">
                    <p className="text-sm text-green-400 break-all">
                      {t('config.upload.selected')}: {srtFile.name}
                    </p>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-gray-800 border border-gray-600 rounded-md p-2 text-white placeholder-gray-500 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                  placeholder={t('config.script.customPromptPlaceholder')}
                  disabled={isProcessing}
                />
              </div>
            </section>
            ...
            <button
              ...
              disabled={isProcessing}
              className="w-full py-3 px-6 text-lg font-bold text-white bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 rounded-md transition-colors"
            >
          ...
          <section className="pt-4">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                {scenes.length > 0 && !isAnalyzing && <p className="text-sm text-gray-400">{t('results.script.subtitle')}</p>}
              </div>
              <div className="flex gap-2">
                <button
                  onClick={handleOpenAllScenesJsonModal}
                  disabled={scenes.length === 0}
                  className="flex items-center gap-2 text-sm bg-blue-900/50 hover:bg-blue-800/50 text-blue-300 px-3 py-1.5 rounded-md disabled:opacity-50 transition-colors"
                >
            ...
            <div className="max-h-[60vh] overflow-y-auto bg-gray-900/50 p-4 rounded-lg text-gray-300 prose prose-invert prose-sm">
              {isAnalyzing && scenes.length === 0 ? (
                <p>{t('status.writingInProgress')}</p>
              ) : (
                <div>
                      ...
                      {scene.title && (
                        <div className="flex justify-between items-center mb-2 pb-1 border-b border-gray-700">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => handleOpenSceneJsonModal(index, scene.jsonConfig)}
                              className="flex items-center gap-1.5 text-xs bg-blue-900/50 hover:bg-blue-800/50 px-2 py-1 rounded-md transition-colors text-blue-300"
                            >
                              <EditIcon className="w-3 h-3" />
                              {t('buttons.editJson')}
                            </button>
                            <button
                              onClick={() => handleSceneCopy(scene, index)}
                              className="flex items-center gap-1.5 text-xs bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded-md transition-colors"
                            >
